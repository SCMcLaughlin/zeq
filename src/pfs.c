...
static int pfs_decompress_index(Pfs* pfs, byte** outData, uint32_t* outLength, uint32_t index)
{
    PfsEntry* ent;
    byte* src;
    byte* dst;
    ...
    dst = (byte*)zeq_malloc(ilen);
    
    if (!dst) return ZEQ_ERR_MEMORY;
...
static void pfs_sort(Pfs* pfs, uint32_t n)
{
    qsort(pfs->entries, n, sizeof(PfsEntry), pfs_sort_by_offset);
}

static int pfs_open_impl(Pfs** outPfs, byte* data, uint32_t length)
{
    Pfs* pfs;
    uint32_t p, n, i;
    PfsHeader* h;
    ...
    pfs = (Pfs*)zeq_calloc(1, sizeof(Pfs));
    if (!pfs) return ZEQ_ERR_MEMORY;
    ...
    i = bit_pow2_greater_or_equal_u32(n);
    
    pfs->entries = (PfsEntry*)zeq_malloc(sizeof(PfsEntry) * i);
    if (!pfs->entries) goto oom;
    
    pfs->hashes = (uint32_t*)zeq_malloc(sizeof(uint32_t) * i);
    if (!pfs->hashes) {
    oom:
        rc = ZEQ_ERR_MEMORY;
        goto fail;
    ...
    while (i < n) {
        ...
        pfs->hashes[i] = hash_string(name, namelen - 1);
        ent = &pfs->entries[i];
        ent->name = name;
...
int pfs_open(Pfs** outPfs, const char* path)
{
    ...
    fp = fopen(path, "rb");
    if (!fp) return ZEQ_ERR_NOT_FOUND;
    ...
    rc = pfs_open_impl(outPfs, data, length);
...
static int pfs_file_index_by_name(Pfs* pfs, const char* name, uint32_t namelen)
{
    uint32_t hash, i;
    ...
    hash = hash_string(name, namelen ? namelen : strlen(name));
    
    for (i = 0; i < pfs->count; i++) {
        if (pfs->hashes[i] != hash)
            continue;
            
        if (strcmp(pfs->entries[i].name, name) == 0)
            return (int)i;
    }
    
    return ZEQ_ERR_NOT_FOUND;
}

const char* pfs_file_name(Pfs* pfs, uint32_t index)
{
    if (pfs && index < pfs->count)
        return pfs->entries[index].name;
    
    return NULL;
}

int pfs_file_data(Pfs* pfs, uint32_t index, byte** data, uint32_t* length)
{
    if (!pfs || !data || !length)
        return ZEQ_ERR_INVALID;
    ...
    return pfs_decompress_index(pfs, data, length, index);
}

int pfs_file_data_by_name(Pfs* pfs, const char* name, uint32_t namelen, uint8_t** data, uint32_t* length)
{
    int index;
    ...
    index = pfs_file_index_by_name(pfs, name, namelen);
